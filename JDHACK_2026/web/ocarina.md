# Ocarina - Level 1
## Introduction
In medieval France, before Joan of Arc became a famous warrior, she discovered an ocarina which had the power to open mysterious doors and reveal secret paths, but only when the right songs were played perfectly.

To prove she was worthy of its power, Jeanne must complete 99 musical tests. Each test requires her to listen carefully to a heavenly song and then play it back exactly as she heard it. The songs get longer and more difficult with each test, starting with just one note and growing to 99 notes in the final challenge.

Your mission is to help her master the ocarina by repeating each sacred melody perfectly. But be careful, if you take too long to play back a song, or if you make a mistake, Jeanne will have to start over from the beginning.
## Challenge Resolution
The source code of the game is:
```js
let ws;
let currentSequence = [];
let playerSequence = [];
...
```
We notice in particular that the challenge uses a `websocket` and **server sends next sequence immediately** and **verification is immediate**. To interact with the websocket, we can use a Python script with the [websockets module](https://websocket.org/guides/languages/python/):
```python
#!/usr/bin/env python
...
def runGame():
    uri = "ws://ocarina1.web03.jeanne-hack-ctf.org/ws"
...
if __name__ == "__main__":
    runGame()
```
We find the flag in the last message:
```flag
[!] Unexpected message: {'type': 'flag', 'data': {'flag': 'JDHACK{0car1n4_m4st3ry_4ch13v3d_99_r0unds!}', 'message': 'Completed 99 rounds!'}}
```
